...
$logsDir = Join-Path $root "logs\platform"
$reportsDir = Join-Path $root "reports\platform"
...
$configPath = Join-Path $configDir "platform_pipeline.json"
$runnerPath = Join-Path $root "run_platform.ps1"
...
<div>{html.escape(started_at)} → {html.escape(finished_at)}</div>
</header>
<main>
<div class="metrics">
...
Set-Content -Path $configPath -Value $configCode -Encoding UTF8
Set-Content -Path $runnerPath -Value $runnerCode -Encoding UTF8

Write-Host ""
Write-Host "Created Platform Orchestrator:"
Write-Host $orchestratorPath
Write-Host $configPath
Write-Host $runnerPath

python -m py_compile $orchestratorPath

if ($LASTEXITCODE -ne 0) {
    throw "Platform Orchestrator compile check failed."
}

Write-Host ""
Write-Host "Compile check passed."
Write-Host "Running safe dry-run validation..."
Write-Host ""

& $runnerPath --dry-run --no-open

if ($LASTEXITCODE -ne 0) {
    throw "Platform dry-run validation failed."
}

Write-Host ""
Write-Host ("=" * 110)
Write-Host "PLATFORM ORCHESTRATOR INSTALLED"
Write-Host ("=" * 110)
Write-Host "Dry-run validation: PASSED"
Write-Host ""
Write-Host "Run the full configured platform with:"
Write-Host ".\run_platform.ps1"
Write-Host ""
Write-Host "Run validation only with:"
Write-Host ".\run_platform.ps1 --dry-run"
Write-Host ""
Write-Host "Configuration:"
Write-Host $configPath
Write-Host ""
Write-Host "Existing source engines were not moved or overwritten."
Write-Host ("=" * 110)