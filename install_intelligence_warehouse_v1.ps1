...
$EnginePath = Join-Path $SrcDir "intelligence_warehouse_engine.py"
$MigrationPath = Join-Path $MigrationDir "002_intelligence_warehouse.sql"
...
$BackupDir = Join-Path $AutomationDir "backups\intelligence_warehouse_$Timestamp"
...
if (Test-Path $EnginePath) {
    Copy-Item $EnginePath (Join-Path $BackupDir "intelligence_warehouse_engine.py") -Force
}
if (Test-Path $MigrationPath) {
    Copy-Item $MigrationPath (Join-Path $BackupDir "002_intelligence_warehouse.sql") -Force
}
...
if ($LASTEXITCODE -ne 0) {
    throw "The Intelligence Warehouse engine failed Python compilation."
}
...
if (Test-Path $ConfigPath) {
    ...
    $Existing = @($Config.steps | Where-Object {
        $_.name -eq "intelligence_warehouse" -or
        $_.module -eq "src.intelligence_warehouse_engine"
    })

    if ($Existing.Count -eq 0) {
        $NewStep = [pscustomobject]@{
            name = "intelligence_warehouse"
            module = "src.intelligence_warehouse_engine"
            required = $true
            enabled = $true
        }
        ...
        for ($Index = 0; $Index -lt $Steps.Count; $Index++) {
            if (
                $Steps[$Index].name -eq "resolution_warehouse" -or
                $Steps[$Index].name -eq "resolution_outcome"
            ) {
        ...
        if ($ResolutionIndex -ge 0) {
            $Steps.Insert($ResolutionIndex, $NewStep)
        }
        else {
        ...
        Write-Host "Added intelligence_warehouse to platform_automation.json"
    }
    else {
        Write-Host "intelligence_warehouse is already configured."
    }

    python -c "import json, pathlib; json.loads(pathlib.Path(r'$ConfigPath').read_text(encoding='utf-8-sig')); print('CONFIG JSON: OK')"
    if ($LASTEXITCODE -ne 0) {
        throw "platform_automation.json is invalid after the update."
    }
}

Write-Host ""
Write-Host "INTELLIGENCE WAREHOUSE INSTALLED"
Write-Host "Backup:    $BackupDir"
Write-Host "Migration: $MigrationPath"
Write-Host "Engine:    $EnginePath"
Write-Host ""
Write-Host "Run next:"
Write-Host "  python -m src.intelligence_warehouse_engine"
Write-Host ""
Write-Host "Then run through automation:"
Write-Host "  .\automation\run_platform.ps1 --only intelligence_warehouse"
Write-Host ""
Write-Host "Finally:"
