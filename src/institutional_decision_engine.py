...
Institutional Decision Engine v1.0
...
DECISION_TABLE = "institutional_decisions"
HISTORY_TABLE = "institutional_decision_history"
RUNS_TABLE = "institutional_decision_runs"
...
def build_decision(
    ...
    explanation = (
        f"{action} with decision score {score:.1f}/100 "
        f"and confidence {confidence:.1f}%. "
        f"Consensus {consensus_score:.1f}, wallet quality "
        f"{wallet_quality:.1f}, trust {trust['trust']:.1f}, "
        f"entry {entry:.1f}, structure {structure:.1f}, "
        f"evolution {evolution:.1f}, data {data_quality:.1f}. "
    )
    ...
    return Decision(
        ...
        title=clean_text(master.get("title")),
