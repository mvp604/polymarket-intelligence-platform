...
def generate_reports(signals: list[MarketSignal], source: str) -> dict[str, Path]:
    ...
    for index, signal in enumerate(signals[:50], start=1):
        lines.extend(
            [
                f"{index}. [{signal.opportunity_grade}] {signal.opportunity_score:.2f} â€” "
                f"{signal.title} â€” {signal.outcome}",
    ...
    for index, signal in enumerate(signals, start=1):
        rows.append(
            ...
            <tr>
              ...
              <td>{signal.conviction_score:.2f}<br><small>Î” {signal.conviction_trend:+.2f}</small></td>
              <td>${signal.combined_value:,.2f}<br><small>Î” ${signal.value_trend:+,.2f}</small></td>
              <td>{signal.average_current_price:.3f}<br><small>Î” {signal.price_trend:+.3f}</small></td>
    ...
    top_cards = "".join(
        f"""
        <div class="card">
          <div class="grade">{html.escape(item.opportunity_grade)} Â· {item.opportunity_score:.1f}</div>
...
<div>Data source: {html.escape(source)} Â· Records: {len(signals)}</div>
</header>
<main>
