...
Institutional Decision Engine v2.2
...
PROFILE_STRUCTURE_WEIGHTS: dict[
    str,
...
PROFILE_EVOLUTION_WEIGHTS: dict[
    str,
...
def score_structure(
    ...
    (
        ...
        PROFILE_STRUCTURE_WEIGHTS,
        profile_name,
    )

    return clamp(
        (100 - conflict) * conflict_weight
        + (100 - reversal) * reversal_weight
        + (100 - volatility) * volatility_weight
        + independence * independence_weight
        + market_depth * depth_weight
    )
...
def score_evolution(
    ...
    (
        ...
        PROFILE_EVOLUTION_WEIGHTS,
        profile_name,
    )

    return clamp(
        evolution * evolution_weight
        + strengthening * strengthening_weight
        + (100 - weakening) * weakening_weight
    )


def score_profile_timing(
    master: Mapping[str, Any],
    profile_name: str = "DEFAULT",
) -> float:
    base_timing = clamp(
    ...
    normalized_profile = clean_text(
        profile_name
    ).upper()

    if normalized_profile == "SPORTS":
        return clamp(
            base_timing * 0.50
            + edge * 0.20
            + chase_resistance * 0.20
            + strengthening * 0.10
        )

    if normalized_profile == "POLITICS":
        return clamp(
            base_timing * 0.50
            + strengthening * 0.20
            + edge * 0.15
    ...
    if normalized_profile == "CRYPTO":
        return clamp(
            base_timing * 0.40
            + chase_resistance * 0.30
            + strengthening * 0.20
            + edge * 0.10
        )

    if normalized_profile == "MACRO":
        return clamp(
            base_timing * 0.48
            + completeness * 0.22
            + edge * 0.17
...
EXPLAINABILITY_VERSION = "2.2"


def build_score_contributions(
...
def format_score_contributions(
    ...
    for component_name in ordered_names:
        ...
        parts.append(
            f"{component_name}="
            f"{safe_float(contributions[component_name]):.2f}"
        )
...
def requirement_failure(
    ...
    return (
        f"{label} "
        f"{actual:.2f}<{required:.2f} "
        f"(short {required - actual:.2f})"
    )


def minimum_count_failure(
    label: str,
    actual: int,
    required: int,
) -> str | None:
    ...
    return (
        f"{label} "
        f"{actual}<{required} "
        f"(short {required - actual})"
    )


def build_buy_failures(
    score: float,
    actionability: float,
    confidence: float,
    entry: float,
    ...
    checks = (
        ...
        requirement_failure(
            "confidence",
            confidence,
            thresholds.buy_confidence,
        ),
        requirement_failure(
            "entry",
            entry,
            thresholds.buy_entry,
        ),
        requirement_failure(
            "structure",
            structure,
            thresholds.buy_structure,
        ),
        requirement_failure(
            "trust",
            trust,
            thresholds.buy_trust,
        ),
        minimum_count_failure(
            "wallets",
            wallet_count,
            thresholds.buy_wallet_count,
        ),
        requirement_failure(
            "data",
            data_quality,
...
def explain_failed_requirements(
    failures: list[str],
) -> str:
    if not failures:
        return "none"
...
def build_decision(
    ...
    score_contributions = build_score_contributions(
        components,
        profile_weights,
    )
    ...
    buy_failures = build_buy_failures(
        score=score,
        actionability=actionability,
        confidence=confidence,
        entry=entry,
    ...
    watch_failures = build_watch_failures(
        score=score,
        actionability=actionability,
        confidence=confidence,
        vetoes=vetoes,
    ...
    distance_to_buy = distance_to_threshold(
        score,
        thresholds.buy_score,
    )

    distance_to_watch = distance_to_threshold(
        score,
        thresholds.watch_score,
    )

    contribution_text = format_score_contributions(
        score_contributions
    )
    ...
    explanation = (
        f"{action} using the "
        f"{scoring_profile.name} v2.2 scoring profile. "
        ...
        f"trust adjustment {trust_adjustment:+.2f}; "
        f"inherited penalty -{inherited_penalty:.2f}; "
        f"final score {score:.2f}. "
        f"Distance to WATCH score: "
        f"{distance_to_watch:.2f}. "
        f"Distance to BUY score: "
        f"{distance_to_buy:.2f}. "
        f"Failed WATCH requirements: "
        f"{explain_failed_requirements(watch_failures)}. "
        f"Failed BUY requirements: "
        f"{explain_failed_requirements(buy_failures)}. "
    )
