...
$enginePath = Join-Path $src "institutional_decision_audit_engine.py"
$runnerPath = Join-Path $root "run_decision_audit.ps1"
...
Write-Host "Running decision audit..."
Write-Host ""

& $runnerPath

if ($LASTEXITCODE -ne 0) {
    throw "Decision audit runner failed."
}

Write-Host ""
Write-Host "Installation and first audit completed."