...
foreach ($Required in @($SrcDir, $AutomationDir, $MigrationDir, $DatabasePath, $EnginePath)) {
...
$BackupDir = Join-Path $AutomationDir "backups\opportunity_schema_repair_$Timestamp"
New-Item -ItemType Directory -Path $BackupDir -Force | Out-Null

Copy-Item $DatabasePath (Join-Path $BackupDir "polymarket.db") -Force
if (Test-Path $MigrationPath) {
    Copy-Item $MigrationPath (Join-Path $BackupDir "003_opportunity_intelligence.sql") -Force
}
Copy-Item $EnginePath (Join-Path $BackupDir "opportunity_intelligence_engine.py") -Force

$RepairPath = Join-Path $SrcDir "repair_opportunity_schema.py"
...
Write-Utf8NoBom -Path $RepairPath -Content $RepairCode
...
'@
Write-Utf8NoBom -Path $MigrationPath -Content $SafeMigration

python -m py_compile $RepairPath
if ($LASTEXITCODE -ne 0) {
    throw "Schema repair script failed Python compilation."
}

Write-Host ""
Write-Host "Running schema repair..."
python -m src.repair_opportunity_schema
if ($LASTEXITCODE -ne 0) {
    throw "Opportunity schema repair failed. Database backup is available at $BackupDir"
}

Write-Host ""
Write-Host "Testing Opportunity Intelligence Engine..."
python -m src.opportunity_intelligence_engine
if ($LASTEXITCODE -ne 0) {
    throw "Opportunity Intelligence Engine still failed after schema repair."
}

Write-Host ""
Write-Host "OPPORTUNITY INTELLIGENCE REPAIR COMPLETE"
Write-Host "Backup: $BackupDir"
Write-Host ""
Write-Host "Next full run:"
