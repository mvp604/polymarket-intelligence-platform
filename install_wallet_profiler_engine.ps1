...
$reportDir = Join-Path $root "reports\wallet_profiler"
...
$databaseBackup = Join-Path $backupDir "polymarket_before_wallet_profiler_$timestamp.db"
Copy-Item $databasePath $databaseBackup -Force

$enginePath = Join-Path $srcDir "wallet_profiler.py"
$runnerPath = Join-Path $root "run_wallet_profiler.ps1"
$manifestPath = Join-Path $srcDir "wallet_profiler.manifest.json"
...
<div>Generated: {payload['generated_at']} · Wallets: {len(profiles)} · Run ID: {run_id}</div>
</header>
<main>
...
def main() -> None:
    ...
    print(f"Database modified:          {'NO' if args.dry_run else 'YES — intelligence tables only'}")
...
{
  "id": "wallet_profiler",
  "name": "Elite Wallet Profiler",
  "version": "1.0.0",
  "runner": "run_wallet_profiler.ps1",
  "enabled": true,
  "required": true,
  "stage": "intelligence",
  "order": 45,
  ...
  "latest_report": "reports/wallet_profiler/latest.html",
  "timeout_seconds": 900
}
'@

if (Test-Path $enginePath) {
    $backup = "$enginePath.backup.$timestamp"
    Copy-Item $enginePath $backup -Force
    Write-Host "Existing profiler backed up:"
    Write-Host $backup
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Wallet Profiler compile check failed."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Wallet Profiler self-test failed."
}
...
if ($LASTEXITCODE -ne 0) {
    throw "Wallet Profiler dry run failed."
}

Write-Host ""
Write-Host ("=" * 110)
Write-Host "ELITE WALLET PROFILER INSTALLED"
Write-Host ("=" * 110)
...
Write-Host ""
Write-Host "Run profiler:"
Write-Host ".\run_wallet_profiler.ps1"
Write-Host ""
Write-Host "Run complete platform:"
